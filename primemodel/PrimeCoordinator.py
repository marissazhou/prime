...
class PrimeCoordinator():
	...
	"""
	output_total_population			= 0 #sum(DBHelper.population_subsets)  
	...
	def __init__(self):
		...
		self.output_total_population	= number_to_string_comma(DBHelper.population_total_size)  # sum up of all populations
	...
	def get_exposure(self, exposure_id, b_mean, b_sd, c_mean, c_sd, b_non_rate, c_non_rate, dist_type, mortalities, outcome_ids, pre_compound_outcome_ids):#id in db
		...
		exposure_bins_rr= DBHelper.exposure_bins_rr.get(e_id) #bins to define the relative risks of the exposure within population. (e.g. salt [1,3,5,...]) 
		exposure_bins 	= DBHelper.exposure_bins.get(e_id) #bins to define the exposure within population (e.g. salt [0-2,2-4,4-6,...]), the upper limit is chosen to store as the bins 
		risks 		= DBHelper.risks.get(e_id) #initial risks for all population subsets of the current exposures, including relative risks for all outcomes related to current exposure and stored in a dictionary like {outcome_id:[relative risks for all population subsets]} 
		...
		populationDistribution = PopDistribution(DBHelper.population_subsets,b_non_rate,b_mean,b_sd,c_non_rate,c_mean,c_sd,exposure_bins,dist_type)
		...
		for o_id in sorted(outcome_ids): # [related outcome id list]
			...
			name 		= DBHelper.get_outcome_name(o_id) # outcome name
			lle 		= DBHelper.exposure_outcome.get(e_id).get(o_id).get('lower_limit_estimate') # lower limit estimate
			ule 		= DBHelper.exposure_outcome.get(e_id).get(o_id).get('upper_limit_estimate') # upper limit estimate
			measure 	= DBHelper.exposure_outcome.get(e_id).get(o_id).get('measure') # measurements 
			outcome 	= PrimeOutcome(name,o_id,e_id,mortality,exposure_bins_rr,outcome_risks,lle,ule,measure) # initiate an outcome object
		...
		exposure 		= PrimeExposure(e_id,outcome_ids,compound_outcome_ids,mortalities,exposure_bins_rr,exposure_bins,outcomes,populationDistribution) # initiate an exposure object 
		return exposure
	...
	def get_exposure_list(self, e_list):
		...
		c_mortalities 		= DBHelper.mortalities # this is the orginal mortality data for chosen population from database. This is to be used as initial mortalities for outcomes. 

		for e in e_list:
		 	e_id 		= int(e.get('e_id')) # exposure id, when get exposure id from database, it is a unicode
			outcome_ids     = DBHelper.exposure_outcome.get(e_id) # {exposure_id:[related outcome ids]}
			...
			measurement	= DBHelper.measurements.get(e_id) # [baseline mean, counterfactual mean, baseline sd, counterfactual sd, baseline non rate, counterfactual non rate]
			...
			if e_id == 5: # for fat, we get parameters like total fat mean etc, we need to calculate cholestrol mean and cholestrol sd based on different fats parameters
				...
				c_mean_value_m	= get_bmi_counterfactual_mean(float(DBHelper.b_bmi_mean), float(DBHelper.b_energy_mean), float(c_energy_mean), float(DBHelper.b_height_mean), float(DBHelper.b_sedentary_rate), float(DBHelper.b_met_mean), float(DBHelper.b_met_mvpa), float(DBHelper.b_met_non_mvpa), float(c_sedentary_rate), float(c_met_mean), float(c_met_mvpa), float(c_met_non_mvpa), True)
				c_mean_value_f	= get_bmi_counterfactual_mean(float(DBHelper.b_bmi_mean), float(DBHelper.b_energy_mean), float(c_energy_mean), float(DBHelper.b_height_mean), float(DBHelper.b_sedentary_rate), float(DBHelper.b_met_mean), float(DBHelper.b_met_mvpa), float(DBHelper.b_met_non_mvpa), float(c_sedentary_rate), float(c_met_mean), float(c_met_mvpa), float(c_met_non_mvpa), False)
				c_sd_value	= float(DBHelper.b_bmi_sd) 
				c_non_rate_value= None 
			...
			if c_non_rate_value != None:
				...
				c_non_rate 	= None 
			dist_type	= DBHelper.exposures.get(e_id).get('dist_type') # unicode
	...
	def get_initial_exposures(self):
		...
		keys 			= DBHelper.exposures.keys()
		for e_id in keys: # get a list of exposures 
			exposure	= DBHelper.exposures.get(exposure_id)	
			outcome_ids 	= DBHelper.exposure_outcome.get(e_id)

			exposure_bins_rr= DBHelper.exposure_bins_rr.get(e_id)
			exposure_bins 	= DBHelper.exposure_bins.get(e_id)
			risks 		= DBHelper.risks.get(e_id)
			dist_type   	= exposures.get('dist_type')
			b_mean 		= DBHelper.measurements.get(e_id)[0]# [[b_means],[c_means],[b_sd],[c_sd],[b_non_rate],[c_non_rate]]
			c_mean 		= DBHelper.measurements.get(e_id)[1] 
			b_sd 		= DBHelper.measurements.get(e_id)[2] 
			c_sd 		= DBHelper.measurements.get(e_id)[3] 
			if len(DBHelper.measurements)<6: # some exposures have no non_rate, such as Fibre and Fats
				non_rate= genearte_list_value(0,len(b_mean)) # all value should be 0 
			else:	# some exposures have non_rate, such as fruit veg etc. 
				non_rate= DBHelper.measurements.get(e_id)[4]
			mortalities	= DBHelper.mortalities
	...
	def refresh_output_baseline_mortality(self):
   	 	...
   	 	:returns: No return
    		"""
		self.output_total_death_baseline	= DBHelper.total_mortality
		self.output_baseline_mortality_num 	= DBHelper.outcome_mortality
	...
	def refresh_output_counterfactual_mortality(self):
		...
		try:
			...
			if None != exposure:
				...
				self.output_total_death_averted		= DBHelper.total_mortality - self.output_counterfactual_mortality_total # total death averted by counterfactual parameters 
			...
			for o_id in exposure.compound_outcome_ids:
				...
				mortality_detail_outcome['name'] 		= DBHelper.get_outcome_name(o_id) # outcome name
				mortality_detail_outcome['b_mortality_sum_db'] 	= DBHelper.outcome_mortality.get(o_id) # baseline mortality from database
				mortality_detail_outcome['c_mortality_sum'] 	= int(round(exposure.c_mortalities_outcome.get(o_id),0)) #final counterfactual mortality for compound exposures
				...
				if len(self.output_all_mortality_age) < 1:
					for i in xrange(1,DBHelper.age_group_num + 1): # i is the age group id
						mortality_detail_age				= {} # one mortality detail for one age group 
						mortality_detail_age['population_subset_id']	= i # age group id 
						mortality_detail_age['age_group']		= DBHelper.get_age_group(i-1) # age group name
						mortality_detail_age['b_mortality_sum_db'] 	= DBHelper.age_mortality.get(i) # baseline mortality from database
			...
			mortality_detail_male['b_mortality_sum_db'] 	= DBHelper.gender_mortality.get('male') # baseline mortality from database
			mortality_detail_male['c_mortality_sum'] 	= int(round(exposure.c_mortalities_gender.get('male'),0)) # final counterfactual mortality for compound exposures

			mortality_detail_female				= {} # one mortality detail for one outcome
			mortality_detail_female['gender'] 		= 'female' # outcome id
			mortality_detail_female['b_mortality_sum_db'] 	= DBHelper.gender_mortality.get('female') # baseline mortality from database
			mortality_detail_female['c_mortality_sum'] 	= int(round(exposure.c_mortalities_gender.get('female'),0)) # final counterfactual mortality for compound exposures
			...
			#age_m= self.output_all_mortality_age
			#gender_m = self.output_all_mortality_gender
			#log = 1/None
	...
	def get_attr_b_death_age(self, a_id):
    		...
    		"""
		#if not DBHelper.is_agegroup_valid(a_id): return -1
		age_deaths_sum = 0
