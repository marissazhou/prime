...
class PrimeExposureRaw():
	...
	def get_c_mortalities(self):
		""" return final counterfactual mortality, same format as DBHelperRaw.mortalities, aka the origianl mortality 
	...
	def get_c_mortalities_age_and_gender(self):
		age_sep 				= DBHelperRaw.age_group_num/2 + 1
		self.c_mortalities_age			= {} # {age_group_id: mortality number} 
		for age_id in range(1, DBHelperRaw.age_group_num + 1): # initiate mortality in all age groups to be 0
		...
		for o_id in keys:
			...
			for age_id in range(1, DBHelperRaw.age_group_num + 1): # 
