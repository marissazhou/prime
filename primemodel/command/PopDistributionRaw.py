...
class PopDistributionRaw:
	def __init__(self, population_subsets, b_non_rate, b_mean, b_sd, c_non_rate, c_mean, c_sd, bins_pop, dist_type):
		...
		'''
		b_log_mean_sds		= logic.get_log_mean_sd(b_mean,b_sd) # returns [log_mean, log_sd]
		c_log_mean_sds		= logic.get_log_mean_sd(c_mean,c_sd) # returns [log_mean, log_sd]
		...
		for i in range(m):
			...
			for j in range(n):
				if j == n-1:#last one
					...
					density		= logic.get_population_density(population_ex_non_rate,upper,lower,mean[i],sd[i],log_mean[i],log_sd[i],self.dist_type,is_last)
				else:
					is_last 	= False 
